...
const RepliesComments = () => {
     ...
     const getReplies=async()=>{
        setLoadReplies(true)
      const res=await axios.get('http://127.0.0.1:8000/api/panel/replies?page='+1/*page*/);
  ...
  const changePageFunc=(page)=>{
   ...
   const submitFunc=async(e)=>{
    if(btnSubmitFunc.current){
      ...
      const reply='reply'
      const res=await http5.post(`/panel/comment`,{reply,text,id})
   ...
   const deleteFunc=async(id)=>{
       if(window.confirm('Delete this reply?')){
           const reply='reply'
           const res=await http5.post(`/panel/delete-comment/`,{reply,id})
   ...
   useEffect(() => {
    ...
    return (
        ...
        <div className='container-fluid top-cont'>
            {loadReplies ? (<p className='spinner-border gray mx-auto mt-5 d-block'></p>)
             : replies&& Array.isArray(replies)&& replies.length>0
              ?
                <>
                <p>All replies  ({total})</p>
                <div className='overflow-auto w-100 pb-2 table-user-parent' >
                    ...
                    <thead className='bg-info fw-bold'>
                        ...
                        {  replies.map((e)=>(
                            <tr key={e.id}    >
                                <td>{e.id}</td>
                                <td className='bg-light w-25 overflow-auto' dir='rtl'><span className='mh-20 d-block'>{e.text}</span></td>
                                <td dir='rtl'>{<GetAdName id={e.item_id} />}</td>
                                <td>{<GetUserName id={e.commentor} />}</td>
                                <td className='bg-light w-25 overflow-auto'><span className='mh-20 d-block'><GetComment id={e.c_id} /></span></td>
                                <td dir='rtl'>{e.date}</td>  
                                <td>
                                    <i onClick={()=>{editFunc(e)}} className='bi bi-pen me-3 text-success'></i>
                                    <i onClick={()=>{deleteFunc(e.id)}} className='bi bi-trash text-danger'></i>
                                </td>
